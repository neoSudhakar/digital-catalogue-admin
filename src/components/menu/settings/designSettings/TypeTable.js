...
const handleDeleteRow = (rowToDelete) => {
    fetch(`http://localhost:8080/api/types/${rowToDelete.id}`, {
      method: 'DELETE',
      headers: {
    ...
    const columnDefs=[
        ...
        { headerName: 'Type', field: 'name', width:450 },
        {
            headerName: "Actions",
            width:700,
            cellRenderer: (params) => (
    ...
    return(
        <div style={{margin: 15}}>
            ...
            <UpdateModal openModal={isModalOpen} closeModal= {handleCloseModal} title={"UPDATE TYPE"}>
                <Type refetchData={refetch} closeModal={handleCloseModal} selectedRow={selectedRow}/>
            </UpdateModal>
        </div>
    )
};