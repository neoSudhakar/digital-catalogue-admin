...
export default function StoneGroup({refetchData, closeModal, selectedRow}) {
  ...
  const initialNameValue= selectedRow ? selectedRow.name: ""; 
    ...
    const {
      ...
      const handleSubmit = (event) => {
        ...
        if(!selectedRow){
        fetch('http://localhost:8080/api/stonegroups', {
            method: 'POST',
            headers: {
        ...
        .catch(error => console.log('error occured!'));
          ...
          fetch(`http://localhost:8080/api/stonegroups/${selectedRow.id}`, {
        method: 'PUT', 
        headers: {
          'Content-Type': 'application/json',
